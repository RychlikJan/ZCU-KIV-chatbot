...
public class GUIController {
	...
	private void actionWithTF() {	
		...
		case 0:
			...
			tf_userArea.clear();
			ta_chatbotArea.appendText("JakÈ je vaöe p¯ÌjmenÌ?\n");
			break;
		case 1:
			lastName = tf_userArea.getText();
			tf_userArea.clear();
			ta_chatbotArea.appendText("JakÈ je vaöe telefonnÌ ËÌslo?\n");
			break;
		case 2:
			phoneNumber = tf_userArea.getText();
			tf_userArea.clear();
			ta_chatbotArea.appendText("Na jakou adresu chcete zboûÌ zaslat?\n");
			break;
		case 3:
	...
	private void actionWithRB() {
		...
		case 0:
			if(rb.getSelectedToggle().equals(rb1)) {
				...
				if(rbButtons[0] == null) {
					...
					ta_chatbotArea.appendText("V˝bornÏ, abychom V·m mohli vybrat skvÏl˝ telefon, pot¯ebujeme nejprve p·r parametr˘. Zvolte prosÌm jak· kontrukce by v·m nejvÌce vyhovovala\n\n");
					updateRB();
					rbOption++;
				}
			}else if(rb.getSelectedToggle().equals(rb2)) {
				ta_chatbotArea.appendText("Omlouv·m se, ale jedinÈ co umÌm je objedn·v·nÌ telefn˘.\n\n");
			}
			break;
		case 1:
			if(skip) {
			...
			if(rbButtons[0] == null) {
				...
				ta_chatbotArea.appendText("TeÔ je na ¯adÏ vybrat v˝robce.\n\n");
				updateRB();
				rbOption++;
			}				
			break;
		case 2:
			if(skip) {
				...
				updateOptions("V˝robce", rb.getSelectedToggle().getUserData().toString());
			}
			findOptions("RozliöenÌ displeje");
			if(rbButtons[0] == null) {
				skip = true;
				rbOption++;
			}else {
				ta_chatbotArea.appendText("TeÔ je na ¯adÏ vybrat rozliöenÌ displeje.\n\n");
				updateRB();
				rbOption++;
			}
			break;
		case 3:
			if(skip) {
				...
				updateOptions("RozliöenÌ displeje", rb.getSelectedToggle().getUserData().toString());
			}
			findOptions("RozliöenÌ fotoapar·tu");
			if(rbButtons[0] == null) {
				skip = true;
				rbOption++;
			}else {
				ta_chatbotArea.appendText("TeÔ je na ¯adÏ vybrat rozliöenÌ fotoapar·tu.\n\n");
				updateRB();
				rbOption++;
			}			
			break;
		case 4:
			if(skip) {
				...
				updateOptions("RozliöenÌ fotoapar·tu", rb.getSelectedToggle().getUserData().toString());
			}
			findOptions("PamÏù RAM");
			if(rbButtons[0] == null) {
				skip = true;
				rbOption++;
			}else {
				ta_chatbotArea.appendText("Jakou velkou pamÏù RAM chcete?\n\n");
				updateRB();
				rbOption++;
			}			
			break;
		case 5:
			if(skip) {
				...
				updateOptions("PamÏù RAM", rb.getSelectedToggle().getUserData().toString());
			}
			...
			if(rbButtons[0] == null) {
				...
				ta_chatbotArea.appendText("Jakou poûadujete kapacitu baterie?\n\n");
				updateRB();
				rbOption++;
			}			
			break;
		case 6:
			...
			if(rbButtons[0] == null) {
				...
				ta_chatbotArea.appendText("PolednÌm parametrem, kter˝ musÌte zvolit je typ displeje?\n\n");
				updateRB();
				rbOption++;
			}			
			break;
		...
		//Sem kdyûtak p¯edËasn˝ vykopnutÌ
		System.out.println(remainingNumberOfPhones);
...
//		}
	...
	void initialize() {
		...
		ta_chatbotArea.appendText("Ahoj, j· jsem ultra super v˝kon˝ chatbot pro objedn·v·nÌ telefon˘. Chcete si s mojÌ pomocÌ koupit nÏjak˝ telefon?\n\n");
		rb1.setText("Ano");
		rb2.setText("Ne");
		rb1.setVisible(true);
