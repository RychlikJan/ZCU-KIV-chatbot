...
public class GUIController {
	...
	private void actionWithTF() {	
		...
		case 2:
			...
			tf_userArea.clear();
			ta_chatbotArea.appendText("Na jakou adresu chcete zboí zaslat?\n\n");
			tfOption++;
			break;
		case 3:
			...
			tf_userArea.clear();
			createOrder();
			ta_chatbotArea.appendText("Gratuluju, Váš vysnìnı telefon je objednán. V následujících dnech vám jej doruèí kurır.\n\n");
			break;
		}
	...
	private void actionWithRB() {
		...
		case 0:
			if(rb.getSelectedToggle().equals(rb1)) {
				...
				if(rbButtons[0] == null) {
					...
					ta_chatbotArea.appendText("Vıbornì, abychom Vám mohli vybrat skvìlı telefon, potøebujeme nejprve pár parametrù. Zvolte prosím jaká kontrukce by vám nejvíce vyhovovala\n\n");
					updateRB();
					rbOption++;
				}
		...
		case 1:
			...
			if(rbButtons[0] == null) {
				...
				ta_chatbotArea.appendText("Teï je na øadì vybrat vırobce.\n\n");
				updateRB();
				rbOption++;
			}				
			break;
		case 2:
			if(skip) {
				...
				updateOptions("Vırobce", rb.getSelectedToggle().getUserData().toString());
			}
		...
		case 5:
			...
			if(rbButtons[0] == null) {
				...
				ta_chatbotArea.appendText("Jakou poadujete kapacitu baterie?\n\n");
				updateRB();
				rbOption++;
			}			
			break;
		case 6:
			...
			if(rbButtons[0] == null) {
				...
				ta_chatbotArea.appendText("Poledním parametrem, kterı musíte zvolit je typ displeje?\n\n");
				updateRB();
				rbOption++;
			}			
			break;
		...
		//Sem kdytak pøedèasnı vykopnutí
		System.out.println(remainingNumberOfPhones);
...
//		}
	...
	void initialize() {
		...
		ta_chatbotArea.appendText("Ahoj, já jsem ultra super vıkonı chatbot pro objednávání telefonù. Chcete si s mojí pomocí koupit nìjakı telefon?\n\n");
		rb1.setText("Ano");
		rb2.setText("Ne");
		rb1.setVisible(true);
